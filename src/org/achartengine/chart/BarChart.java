...
public class BarChart extends XYChart {
    ...
    protected ClickableArea[] clickableAreasForPoints(List<Float> points,
        ...
        for (int i = 0; i < length; i += 2) {
            ...
            if (mType == Type.STACKED) {
                ...
                float startX = x - (seriesNr * halfDiffX) +
                    (seriesIndex * 2 * halfDiffX);
                ret[i / 2] = new ClickableArea(new RectF(startX,
                            Math.min(y, yAxisValue), startX + (2 * halfDiffX),
                            Math.max(y, yAxisValue)), values.get(i),
                        values.get(i + 1));
            }
        }
    ...
    public void drawSeries(Canvas canvas, Paint paint, List<Float> points,
        ...
        int startIndex) {
        int seriesNr = mDataset.getSeriesCount();
        int colorLength = seriesRenderer.getColors().length;
        int length = points.size();
        ...
        for (int i = 0; i < length; i += 2) {
            if (seriesRenderer.isMultipleColorsEnabled()) {
            	if(j>colorLength-1){
            		paint.setColor(seriesRenderer.getColors()[j%colorLength]);
            		j++;
            	}else{
    ...
    protected void drawBar(Canvas canvas, float xMin, float yMin, float xMax,
        float yMax, float halfDiffX, int seriesNr, int seriesIndex, Paint paint) {
        ...
        if (mType == Type.STACKED) {
            ...
            float startX = xMin - (seriesNr * halfDiffX) +
                (seriesIndex * 2 * halfDiffX);
            drawBar(canvas, startX, yMax, startX + (2 * halfDiffX), yMin,
                scale, seriesIndex, paint);
        }
    }
    ...
    private void drawBar(Canvas canvas, float xMin, float yMin, float xMax,
        ...
        if (renderer.isGradientEnabled()) {
            float minY = (float) toScreenPoint(new double[] {
                        0, renderer.getGradientStopValue()
                    }, scale)[1];
            float maxY = (float) toScreenPoint(new double[] {
                        0, renderer.getGradientStartValue()
                    }, scale)[1];
    ...
    private int getGradientPartialColor(int minColor, int maxColor,
        ...
        int r = Math.round((fraction * Color.red(minColor)) +
                ((1 - fraction) * Color.red(maxColor)));
        int g = Math.round((fraction * Color.green(minColor)) +
                ((1 - fraction) * Color.green(maxColor)));
        int b = Math.round((fraction * Color.blue(minColor)) +
                ((1 - fraction) * Color.blue((maxColor))));
    ...
    protected void drawChartValuesText(Canvas canvas, XYSeries series,
        SimpleSeriesRenderer renderer, Paint paint, List<Float> points,
        ...
        for (int i = 0; i < length; i += 2) {
            ...
            if (!isNullValue(value)) {
                ...
                if (mType == Type.DEFAULT) {
                    x += ((seriesIndex * 2 * halfDiffX) -
                    ((seriesNr - 1.5f) * halfDiffX));
                }

                if (value >= 0) {
                    drawText(canvas,
                        getLabel(renderer.getChartValuesFormat(), value), x,
    ...
    public void drawLegendShape(Canvas canvas, SimpleSeriesRenderer renderer,
        ...
        canvas.drawRect(x, y - halfShapeWidth, x + SHAPE_WIDTH,
            y + halfShapeWidth, paint);
    }
    ...
    protected float getHalfDiffX(List<Float> points, int length, int seriesNr) {
        ...
        return (float) (halfDiffX / (getCoeficient() * (1 +
